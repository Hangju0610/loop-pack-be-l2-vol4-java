...
| `waiting-queue-load.js` | 대기열 처리량 초과: 스파이크(ramp)/지속 유입 + 폴링 + 주문-결제-토큰소비 |
| `run-waiting-queue.sh` | 위 스크립트 실행기: redis-exporter 를 테스트 동안만 기동 + k6→Prometheus remote-write |
...
# 권장: 실행기 사용 — redis-exporter(대기열 길이/토큰 수)를 테스트 동안만 띄우고
#       k6 메트릭을 remote-write 해 Grafana 'WaitingQueue' 대시보드에서 실시간 관찰.
./k6/run-waiting-queue.sh                                          # S1: 10,000명 / 30초 ramp
./k6/run-waiting-queue.sh -e USERS=1000                            # 축소 리허설
./k6/run-waiting-queue.sh -e SCENARIO=saturation -e RATE=300       # S2: 유입 > 발급 지속

# 모니터링 없이 k6 만 (콘솔 요약만 필요할 때)
