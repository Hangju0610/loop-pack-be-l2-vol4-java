...
- 날짜: 2026-05-29
- 상태: 대체됨 (superseded by [ADR-038](./038-like-count-event-separation.md), 2026-07-01)

> ⚠️ **이 결정은 [ADR-038](./038-like-count-event-separation.md)로 대체되었습니다.** 좋아요 원장과
> `like_count` 집계를 단일 트랜잭션으로 묶던 원자성 결정은, 파생 통계(집계)의 실패가 핵심 행위(좋아요)를
> 무효화하는 문제로 인해 **인프로세스 이벤트 기반 최종 일관성**으로 전환되었습니다. 아래는 히스토리 보존용입니다.
