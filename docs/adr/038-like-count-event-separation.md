...
3. **[적용됨] 커넥션 2배 점유** — `@Async("likeCountExecutor")` 전환으로 해소(전용 ThreadPoolTaskExecutor, core 2 / max 4 / queue 100). 포화 시 거부 정책은 **로그 후 폐기**다 — `CallerRunsPolicy`는 거부된 집계를 AFTER_COMMIT 시점의 요청 스레드에서 인라인 실행해 커넥션 2배 점유·요청 지연을 재발시키므로 채택하지 않는다. 집계는 유실 허용이라 요청 스레드 보호를 우선하며, 폐기된 delta는 follow-up#1 재계산 배치로 교정한다. 트레이드오프 항목 참조.
