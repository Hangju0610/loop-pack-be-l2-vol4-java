...
원 요구가 명시적으로 느슨한 결합이므로 B. 유실 방지(재처리/아웃박스)는 **Transactional Outbox + Kafka 도입 스텝(후속)에서 해소**하고, 본 스텝은 in-process `ApplicationEvent`로만 진행한다.

> ⚠️ **알려진 비내구성 구멍 (성공 경로, 수용됨).** 본 스텝의 in-process 이벤트는 트랜잭션 경계를 넘어 내구적이지 않다. 결제가 SUCCESS로 커밋된 뒤 AFTER_COMMIT 리스너 실행 전에 앱이 중단되거나 리스너가 실패하면 이벤트가 유실된다. 게다가 재-Poll 경로의 `settle`은 first-wins 멱등이라 이미 확정된 결제에서 early return하여 **이벤트를 재발행하지 않는다** → 원리상 자동 복구 경로가 없다. 결과는 "결제는 됐으나 주문 PENDING·쿠폰 RESERVED 영구 고착". 실패 경로(자원이 묶였다가 cleanup으로 회수 가능)보다 **성공 경로(고객이 결제했으나 주문 미완료)의 심각도가 비대칭적으로 높다.** 이 구멍은 아래 "해소 계획"의 Outbox 도입 전까지 **명시적으로 감수**한다.

**해소 방향 (후속 스텝, 미설계):** Transactional Outbox 패턴을 유력한 방향으로 본다 — 결제 확정과 이벤트 적재를 같은 트랜잭션에 커밋하고 별도 릴레이가 Kafka로 at-least-once 발행. 단 이는 *발행*의 내구성만 확보할 뿐, 핸드오프 전체가 닫히려면 **소비자 멱등 + 재조정(reconciliation)** 이 함께 설계되어야 한다. 구체 설계·검증은 후속 스텝의 과제이며 본 ADR은 이를 확정된 해결책으로 단정하지 않는다.
...
### 트레이드오프 (수용)
- **[내구성 구멍 — 성공 경로, 심각]** 결제 SUCCESS 커밋 후 in-process 이벤트가 유실되면(크래시·리스너 실패) 자동 복구 경로가 없다(`settle` 멱등이 재발행을 막음). "결제됐으나 주문 미완료"가 영구 고착될 수 있다. → **Transactional Outbox + Kafka 도입 스텝에서 해소**(그 전까지 감수, 유실은 로깅·경보로 감지).
- 결제 커밋 후 보상 리스너 실패 시 쿠폰·재고 상태가 어긋날 수 있다(실패 경로는 cleanup으로 회수 가능한 양성 구멍).
...
## 후속 계획 / 재검토 시점
1. **[예정] Transactional Outbox + Kafka 도입** — 성공 경로 내구성 구멍(위 트레이드오프)을 완화하려는 **계획된 다음 스텝**(미설계). 발행 내구화(동일 TX 적재 + 릴레이 at-least-once)에 더해 소비자 멱등·재조정을 함께 설계해야 실효가 있다. 이때 오케스트레이션→코레오그래피(결정 4-B), 얇은→두꺼운 이벤트(결정 3-B)도 함께 재검토.
2. **재결제 요구 등장** — 되돌린 주문 재사용(결정 5) 재검토.
