...
**해소 계획 (후속 스텝):** Transactional Outbox 패턴으로 결제 확정과 이벤트 적재를 **같은 트랜잭션**에 커밋하고, 별도 릴레이가 Kafka로 at-least-once 발행 → 소비자 멱등 처리. 이로써 성공/실패 경로 모두 내구적·재전달 가능해진다.
...
## 후속 계획 / 재검토 시점
1. **[예정] Transactional Outbox + Kafka 도입** — 성공 경로 내구성 구멍(위 트레이드오프)을 해소하는 **계획된 다음 스텝**. 결제 확정과 이벤트 적재를 동일 TX에 커밋 후 릴레이가 Kafka로 at-least-once 발행, 소비자 멱등. 이때 오케스트레이션→코레오그래피(결정 4-B), 얇은→두꺼운 이벤트(결정 3-B)도 함께 재검토.
