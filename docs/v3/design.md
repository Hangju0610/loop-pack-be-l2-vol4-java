...
회원가입 (POST /api/v1/users)
  └── 로그인 (이후 모든 요청에 X-Loopers-LoginId / X-Loopers-LoginPw 헤더 포함)
...
```
X-Loopers-Ldap: loopers.admin 헤더 포함
  ├── 브랜드 관리
  │     ├── 목록/단건 조회, 등록, 수정, 삭제 (/api-admin/v1/brands)
  ├── 상품 관리
  │     ├── 목록/단건 조회, 등록, 수정, 삭제 (/api-admin/v1/products)
  └── 주문 관리
        └── 목록/단건 조회 (/api-admin/v1/orders)
```
...
인증은 Spring MVC `HandlerInterceptor`로 처리하며, `interfaces` 레이어에 위치한다.

### User 인증 (`UserAuthInterceptor`)

```
요청 수신
  → X-Loopers-LoginId 헤더 추출
  → X-Loopers-LoginPw 헤더 추출
  → UserService.getByLoginId(loginId) → 없으면 401
  → BCrypt.checkpw(loginPw, user.password) → 불일치 시 401
  → request attribute에 userId 저장 → Controller에서 추출
```

- `/api/v1/**` 경로에 적용 (단, 회원가입 `/api/v1/users` POST는 제외)
- Controller는 `@RequestHeader("X-Loopers-LoginId")` 또는 request attribute로 userId를 받는다.

### Admin 인증 (`AdminAuthInterceptor`)

```
요청 수신
  → X-Loopers-Ldap 헤더 추출
  → "loopers.admin" 값과 비교 → 불일치 시 403
```

- `/api-admin/v1/**` 경로 전체에 적용.
...
| Facade | `@Transactional` 미적용 — Service에 위임 |
| Service (조회) | `@Transactional(readOnly = true)` |
| Service (쓰기) | `@Transactional` |

- 트랜잭션 경계는 항상 **Service** 레이어에서 시작한다.
- Facade가 여러 Service를 호출할 때 각 Service의 트랜잭션은 독립적으로 동작한다.
- 예외: 주문 생성(`OrderService.createOrder`)처럼 쓰기 작업이 원자적으로 묶여야 하는 경우, 해당 Service 메서드 하나가 `@Transactional`로 전체를 관리한다.

---

## 15. 연쇄 삭제 정책

모든 삭제는 Soft Delete(`deleted_at = now()`)이며, 연쇄 삭제는 Facade 레이어에서 오케스트레이션한다.

```
Brand 삭제
  └── Product 연쇄 soft delete (BrandFacade → ProductService.deleteAllByBrand)
        └── ProductInventory 연쇄 soft delete (ProductInventoryService.deleteByProduct)
        └── Like 연쇄 soft delete (LikeService.deleteAllByProduct)

Product 삭제
  ├── ProductInventory soft delete (ProductInventoryService.deleteByProduct)
  └── Like 연쇄 soft delete (LikeService.deleteAllByProduct)
...
> Like 연쇄 삭제 시 `like_count` 차감은 수행하지 않는다. 상품 자체가 삭제되므로 `like_count` 정합성 유지가 불필요하다.

---

## 16. Soft Delete 범위 및 조회 정책

`BaseEntity`의 `deletedAt` 컬럼으로 모든 엔티티의 Soft Delete를 관리한다.

| 엔티티 | 조회 필터 | 예외 |
|---|---|---|
| Brand | `deleted_at IS NULL` | 없음 |
| Product | `deleted_at IS NULL` | 없음 |
| ProductInventory | `deleted_at IS NULL` | 없음 |
| Like | `deleted_at IS NULL` (active 조회) | 좋아요 재등록 시 `deleted_at` 포함 전체 조회 (Restore 패턴, ADR-008) |
| Order | `deleted_at IS NULL` | 없음 |

- Repository 구현체에서 `deleted_at IS NULL` 조건을 명시적으로 포함한다. Hibernate Filter나 `@Where`는 사용하지 않는다 — 명시적 제어를 우선한다.
...
- 모든 목록 조회 API는 위 기본값을 따른다.
- 응답 구조는 `{ content, page, size, totalElements }` 로 통일한다.
- 예외: `GET /api/v1/orders` — `startAt` / `endAt` 날짜 필터와 함께 사용 (ADR-010).
