...
- **단점**: Admin CRUD API, 인증 토큰 관리 등 추가 구현 범위가 크다. 요구사항에 없는 기능을 구현하는 오버엔지니어링이다.
...
실제 운영 환경 적용 시 고정값 방식은 보안상 취약하다. JWT 기반 Admin 토큰 발급, LDAP 실제 연동, 또는 Spring Security 도입으로 전환이 필요하다.
