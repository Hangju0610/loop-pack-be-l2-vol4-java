...
| purchase_count 집계 | commerce-streamer `OrderEventsConsumer`가 `PaymentCompleteEvent` 소비 후 product_metrics upsert |
...
- `PaymentCompleteEvent` → `product_metrics.purchase_count` +1
...
[commerce-streamer (Kafka Consumer, group: product-metrics)]
  ...
  ├── [같은 TX] event_handled INSERT (outbox_event_id, 'product-metrics') — 중복 시 skip
  ├── [같은 TX] product_metrics UPSERT (purchase_count +1)
  └── TX 커밋
...
private void confirmSuccess(PaymentEntity payment) {
    ...
    outboxRepository.save(OutboxEvent.of(event, "order-events", payment.getOrderId()));  // streamer → purchase_count
...
interfaces/consumer
  ...
  OrderEventsConsumer       @KafkaListener(order-events, group=product-metrics)
                              → PaymentCompleteEvent만 product_metrics.purchase_count 집계
