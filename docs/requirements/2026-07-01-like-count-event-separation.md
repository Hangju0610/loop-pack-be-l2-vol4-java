...
- AC-7: **집계 실패가 없는 정상 경로에서** 동시 좋아요/취소 시 `like_count`가 성공 횟수와 정확히
  일치(lost update 없음). 집계 실패(FR-4)가 발생하면 해당 delta는 유실되어 근사치가 되며, 정합성
  복구는 follow-up(재계산 배치)에 위임한다 — 즉 `like_count`는 **강한 일관성이 아니라 근사 집계치**다.
