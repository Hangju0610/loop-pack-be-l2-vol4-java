...
- **완화 방향은 후속 스텝의 Transactional Outbox + Kafka**(미설계). 발행 내구화만으로는 부족하고 소비자 멱등·재조정을 함께 설계해야 실효가 있으며, 본 스텝에서 확정된 해결책으로 단정하지 않는다. 상세: [ADR-036](../adr/036-payment-event-compensation.md).
